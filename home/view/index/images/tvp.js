...
var tvp=window.tvp||{};var TenVideoPlayer=window.tvp;tvp.lastModify="$Date: 2013-07-26 16:17:59 +0800 (五, 2013-07-26) $";tvp.ver="$Rev: 32237 $";tvp.lastChangedBy="$LastChangedBy: popotang $";tvp.path={base:"http://qzs.qq.com/tencentvideo_v1/js/tvp/",hls_video_cgi:"http://vv.video.qq.com/gethls?otype=json&format=2",html5_video_cgi:"http://vv.video.qq.com/geturl?otype=json"}
...
var url="http://rcgi.video.qq.com/web_report?cmd=2551&";reportUrl(url+r.join("&"));}}})()
...
return;var str=this.getFlashHTML();el.innerHTML=str;flashobj=tvp.$.browser.ie?document.getElementById(this.playerid):document.embeds[this.playerid];$me.onwrite(curVideo.getVid());}
this.getPlayer=function(){return flashobj;}
this.play=function(video){if(!flashobj){throw new Error("未找到视频播放器对象，请确认flash播放器是否存在");}
if(tvp.$.isUndefined(video)&&tvp.$.isFunction(flashobj.setPlaytime)){flashobj.setPlaytime(pauseTime);pauseTime=-1;$me.isStartPlay=true;return;}
if(!video instanceof tvp.VideoInfo){throw new Error("传入的对象不是tvp.VideoInfo的实例");}
$me.isStartPlay=false;var vstart=0,vend=0,tagstart=0,tagend=0;if(video.getIdx()==0){vstart=video.getPrefix()||0;vend=video.getEndOffset()||0;}
else{tagstart=video.getTagStart();tagend=video.getTagEnd();}
...
if(typeof flashobj.loadAndPlayVideoV2=='function'){flashobj.loadAndPlayVideoV2(videoInfo);}}
else if(video.getTagEnd()-video.getTagStart()>0){flashobj.attractionUpdate(video.getTagStart(),video.getTagEnd(),extid);}
...
return;if(!!flashobj&&tvp.$.isFunction(flashobj.getPlaytime)&&tvp.$.isFunction(flashobj.pauseVideo)){pauseTime=flashobj.getPlaytime();flashobj.pauseVideo();$me.isStartPlay=false;}}
...
return"v1010";}
function getPlatform(){var ua=navigator.userAgent;if(/MicroMessenger/i.test(ua)){if(/iphone/i.test(ua)){return 60401;}
return 60301;}
return 0;}
function report(step,val,extData){if(isNaN(val)||val<=0||val>9000000){return;}
var url="http://rcgi.video.qq.com/report/play?";var r=[],pa={"step":step,"val":val,"version":"HTML5-20130227","ctime":tvp.$.getISOTimeFormat(),"vid":curVid,"rid":reportRid,"pid":reportPid,"platform":getPlatform()};if(typeof extData=="object"){tvp.$.extend(pa,extData);}
tvp.$.extend(pa,videodata);for(var p in pa){var v=pa[p];if(isNaN(v)){v=encodeURIComponent(""+v);}
r.push(p+"="+v);}
url+=r.join("&");tvp.report(url);}
function getVideoUrlByVid(vid,hls,callbacks){function error(errCode){if(hls){getVideoUrlByVid(vid,false,callbacks);}
else if(tvp.$.getType(callbacks)=="object"&&tvp.$.isFunction(callbacks.onError)){callbacks.onError(errCode);}
...
if(!tvp.$.isString(vid))
return;var isPay=curVideo.getPay()>0?1:0;reportRid=tvp.$.createGUID(48);reportPid=tvp.$.createGUID(48);reportTimer["step_1009"]=new TimerObject();var reportData={val1:0,val2:0},ajaxTimelong=0;tvp.$.ajax({"url":(hls&&!isPay)?tvp.path.hls_video_cgi:tvp.path.html5_video_cgi,"data":{"vid":vid,"charge":curVideo.getPay()>0?1:0},"dataType":"jsonp","success":function(json){ajaxTimelong=reportTimer["step_1009"].getTimelong();if(!json||!json.s){error(50);return;}
else if(json.s!="o"){error(json.em||50);return;}
else if(!json.vd||!json.vd.vi||!tvp.$.isArray(json.vd.vi)){error(68);return;}
...
continue;if(!!json.vd.vi[i].url){var d=json.vd.vi[i];videourl.push(d.url);try{videodata.duration=parseInt(d.dur);videodata.vt=d.vt;videodata.vurl=d.url.substr(0,d.url.indexOf("?"));videodata.bt=curVideo.getTimelong()||videodata.duration;}
catch(e){}
break;}}
...
return;try{if(!!videoObj){videoObj.currentTime=v;}}
catch(e){setTimeout(function(){setCurrentTime(v)},50);}}
...
el+=(el.indexOf("?")>0?"&":"?")+"sdtfrom="+getMp4Key();str+=['<source src="',el,'"></source>'].join("");});}
else if(tvp.$.isString(videoUrl)){if(videoUrl.indexOf(".m3u8")>0){hls=true;}
videoUrl+=(videoUrl.indexOf("?")>0?"&":"?")+"sdtfrom="+getMp4Key();str+=['<source src="',videoUrl,'"></source>'].join("");}
$videoTag.bind("canplaythrough",function(e){var offset=curVideo.getTagStart()||curVideo.getHistoryStart()||0;if(offset>0){if(hls){setTimeout(function(){setCurrentTime(offset);},500);}
else{setCurrentTime(offset);}}
$videoTag.unbind("canplaythrough");});try{videoObj.pause();}
catch(e){};videoObj.innerHTML=str;tvp.$("#"+playerid+" > source").unbind("error");tvp.$("#"+playerid+" > source").bind("error",function(){if(!reportTimer["step_6"])
...
video=video||curVideo;if(!!curVid&&curVid==video.getVid()&&video.getIdx()>0&&video.getTagStart()>0){setCurrentTime(video.getTagStart());}
else{$me.playVideoByVid(video.getVid());}
if(typeof _flash_view_history=="function"){if(curVideo.getIdx()>0){_flash_view_history(0,0,curVideo.getTimelong());}
else{_flash_view_history(-1,0,curVideo.getTimelong());}
...
catch(e){}},reportHistoryTimer);};initReportParam();$me.setCurVideo(video);if(!!isOnChange){$me.onchange(video.getFullVid());}};this.showPlayer=function(){if(!videoObj)
return;var widStr=$me.width+"";if(widStr.indexOf("%")>-1||widStr.indexOf("px")>-1){videoObj.style.width=$me.width;}
else{videoObj.style.width=$me.width+"px";}
var hiStr=$me.height+"";if(hiStr.indexOf("%")>-1||hiStr.indexOf("px")>-1){videoObj.style.height=$me.height;}
else{videoObj.style.height=$me.height+"px";}
isPlayerHiding=!!0;}
...
tvp.$.extend(sendData,extData);tvp.$.ajax({type:"GET",url:"http://zb.s.qq.com/getproginfo.fcgi",data:sendData,dataType:"jsonp",error:function(){$me.onError();$me.onComplete();},success:function(json){$me.onSuccess(json);$me.onComplete();}});}},getCurChannelInfo:function(cnlId){tvp.$.ajax({type:"get",url:"http://sns.video.qq.com/fcgi-bin/dlib/dataout_pc?otype=json&auto_id=191",data:{cid:cnlId},dataType:"jsonp",success:function(json){if(!!json&&!tvp.$.isUndefined(json["channel"])){var channel=json["channel"];tvp.livehub.g_curCnlInfo.cnlId=cnlId;tvp.livehub.g_curCnlInfo.cnlName=channel["cname"];tvp.livehub.g_curCnlInfo.prmInfo=channel["cur_ptime"]+"|"+channel["cur_pname"]}else{tvp.livehub.g_curCnlInfo={};}}});}}
...
tvp.MP4Link.prototype=new tvp.BasePlayer();tvp=tvp||{};var QQLive=QQLive||{ver:"$Rev: 32237 $",curSSO:"",lastmodify:"$Date: 2013-07-26 16:17:59 +0800 (五, 2013-07-26) $"};var QQLiveSetup=QQLive;QQLive.log=tvp.log;QQLive.debug=function(msg){var funName="";if(msg.indexOf("[")<0&&!!arguments&&!!arguments.callee&&!!arguments.callee.caller){var caller=arguments.callee.caller;funName=QQLive.debug.getFunName(caller)}
...
break;}
case MSG.OCX_EVT_GET_QQ:{var uin=tvp.common.getUin(),skey=tvp.common.getSKey();if(!!tvp.$.userAgent.ie){qqliveocxshell.setParam("uin",uin);qqliveocxshell.setParam("skey",skey);}
...
var uin=tvp.common.getUin(),skey=tvp.common.getSKey();qqliveocxshell.SetParam("uin",""+uin);qqliveocxshell.SetParam("skey",""+skey);qqliveocxshell.SetParam("width",$me.width+"");qqliveocxshell.SetParam("height",$me.height+"");var _oid=tvp.$.getUrlParam("oid");if(!!_oid){qqliveocxshell.SetParam("oid",""+_oid);}
...
if(video instanceof tvp.VideoInfo){var sso=getSSOByVideo(video);$me.getCtrl().Play(sso);saveCurVideo(video);$me.onchange(video.getFullVid());}};this.callback=function(){};this.pause=function(){_ctrl.Pause();}};tvp.OcxPlayer.maxId=0;tvp.OcxPlayer.prototype=new tvp.BasePlayer();tvp.$.extend(tvp.OcxPlayer,{showNoticeTips:function(eleId,data){var ele=tvp.$.isString(eleId)?tvp.$.get(eleId):eleId;if(!ele){return;}
...
tvp.report({"type":1,"host":h,"url":document.location.href,"ua":navigator.userAgent,"ver":"121108","str1":playername,"int1":supportMP4?1:0});}
...
tvp.Player.prototype=new tvp.BasePlayer();tvp.Player.instance=[];/*  |xGv00|8c4c3b7f1205490a3deee84c4718636b */